...
const countComment =
  Number.isInteger(process.env.COMMENT_COUNT) || defaultCount;
...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    const article_seed = Array.from({ length: countArticl }, (_, i) => ({
      ...
      content:
        `${i} ` +
        Array(Math.floor(Math.random() * maxSymbols + 1))
          .fill("bla")
          .join(" "),
    }));

    // generating array comments
    const comment_seed = Array.from(
      { length: countComment * countArticl },
      (_, i) => ({
        // id: startId + i,
        articleId: startId + (i % countComment),
        content:
          "G" +
          Array(Math.floor(Math.random() * maxSymbols + 2))
            .fill("O")
            .join("") +
          "L",
      }),
